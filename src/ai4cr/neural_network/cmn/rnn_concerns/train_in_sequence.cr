    ...
    module Cmn
      module RnnConcerns
        module TrainInSequence
          def train_and_guess_in_sequence(io_sets_pairs)
            # io_sets_pairs = split_for_training(training_data)

            train_in_sequence(io_sets_pairs)

            next_inputs = shifted_inputs(io_sets_pairs.last)

            eval(next_inputs)
          end

          def train_in_sequence(io_sets_pairs)
            io_sets_pairs.each do |io_pair|
