...
module TestHelperSpectator
  def assert_approximate_equality(expected, real, delta = 0.01, message = "")
    if expected.responds_to?(:abs) && real.responds_to?(:abs)
      expect(real).to be_close(expected, delta)
    else
  ...
  def assert_approximate_inequality(expected, real, delta = 0.01)
    if expected.responds_to?(:abs) && real.responds_to?(:abs)
      expect(real).not_to be_close(expected, delta)
    else
      expect(real).not_to eq(expected)
    end
  end
  ...
  def check_guess(next_guess, expected_label)
    expect(result_label(next_guess)).to eq(expected_label)
  end
end
